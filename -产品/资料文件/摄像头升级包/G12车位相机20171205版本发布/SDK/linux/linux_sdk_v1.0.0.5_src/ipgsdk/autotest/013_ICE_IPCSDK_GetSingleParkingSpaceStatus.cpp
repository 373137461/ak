...
int main(int argc, char *argv[])
{
	...
	if (1 == result)
	{
		...
		else
		{
			...
			{
				ICE_U32 u32Num = 0, u32PicLen = 0;
				ICE_PARKING_SPACE_OUTPUT_S stResult;
				
				memset(&stResult, 0, sizeof(stResult));
				result = ICE_IPCSDK_GetSingleParkingSpaceStatus(hSDK, u32Index, &u32Num, 
					&stResult, pcPicData, BUF_SIZE, &u32PicLen, NULL, 0, NULL);
				if (1 != result)
				{
					printf("获取单个车位状态失败\n");
					break;
				}
				else if ((u32Num < 1) || (u32Num > 3))
				{
					printf("车位数无效\n");
					result = 0;
					break;
				}
				else if ((ICE_IPG_EVENT_TYPE_NOON != stResult.enParkingPlaceStatus) && 
					(ICE_IPG_EVENT_TYPE_HAVE_CAR != stResult.enParkingPlaceStatus) && 
					(ICE_IPG_EVENT_TYPE_EMPTY_PLACE != stResult.enParkingPlaceStatus) && 
					(ICE_IPG_EVENT_TYPE_OCCUPY_MULTIPLACE != stResult.enParkingPlaceStatus) && 
					(ICE_IPG_EVENT_TYPE_DISABLED != stResult.enParkingPlaceStatus))
				{
					printf("车位状态无效\n");
					result = 0;
					break;
				}
				else if ((stResult.stPlateInfo.flConfidence > 0) && (0 == plate2int(stResult.stPlateInfo.as8PlateNum)))
				{
					printf("车牌号码无效\n");
					result = 0;
					break;
				}
				else if ((stResult.stPlateInfo.ePlateColor < 0) || (stResult.stPlateInfo.ePlateColor > 7))
				{
					printf("车牌颜色无效\n");
					result = 0;
					break;
				}
				else if ((stResult.s32CarType < 0) || (stResult.s32CarType > 2))
				{
					printf("车位类型无效\n");
					result = 0;
					break;
				}
				else if ((stResult.s32EventType < 0) || (stResult.s32EventType > 2))
				{
					printf("事件类型无效\n");
					result = 0;
					break;
