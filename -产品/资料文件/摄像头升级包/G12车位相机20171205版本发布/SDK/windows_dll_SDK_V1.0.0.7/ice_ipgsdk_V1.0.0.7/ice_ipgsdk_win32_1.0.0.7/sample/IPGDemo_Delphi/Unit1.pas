...
begin
  ...
  SystemCfg.setValue(nToTalSpaceNum[index], nSpaceEn, nSpaceType, strRemoteIP, nRemoteEnable, strControlIP, nControlEnable, stHttpParam);
  if SystemCfg.ShowModal = mrCancel then
     exit;
  FillChar(stHttpParam, SizeOf(http_conf_t), 0);
  SystemCfg.getValue(nSpaceEn, nSpaceType, strRemoteIPDst, nRemoteEnable, strControlIPDst, nControlEnable, stHttpParam);
  for i := 0 to (nToTalSpaceNum[index]-1) do
  begin
  ...
  ICE_IPCSDK_SetCGIParam(hSDK[index], @stHttpParam);
