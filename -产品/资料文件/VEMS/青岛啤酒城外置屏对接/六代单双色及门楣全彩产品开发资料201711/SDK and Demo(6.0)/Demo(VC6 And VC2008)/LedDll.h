...
{
	int		LEDType;				//LED类型	0.为所有6代单色、双色、七彩卡,		1.为所有6代全彩卡
	int		SendType;				//通讯方式	0.为Tcp发送（又称固定IP通讯）,		1.广播发送（又称单机直连）		2.串口通讯		3.磁盘保存
...
struct PLAYPROP 
{
	int			InStyle;	//入场特技值（取值范围 0-39）
	int			OutStyle;	//退场特技值（现无效，预留，置0）
	int			Speed;		//特技显示速度(取值范围1-255)
	int			DelayTime;	//页面留停时间(1-65535)
};
      /*  特技值对应
            0=立即显示
            1=随机
